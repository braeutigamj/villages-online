...
class reload {
  ...
  function bh($hid) {
    ...
    $result = $db->fetch("SELECT `what` FROM `build` WHERE village = '$hid'");
    $btimes = array();
    while($build = $result->fetch_array()) {
        if(isset($btimes[$build['what']])) {
          $btimes[$build['what']]++;
        } else {
          $btimes[$build['what']] == $village[$build['what']];
        }
      $bh += round($building->food[$build['what']]*$btimes[$build['what']]*0.224);
    }
  ...
  */
  ...
  //Berechnet die Arbeiter eines Dorfes neu
  function arbeiter($hid) {
    global $config, $db;
    $village = $db->assoc("SELECT * FROM village WHERE id = '$hid'");
    $arbwood = $village['wood'] * $config['worker'];
    $arbclay = $village['clay'] * $config['worker'];
    $arbiron = $village['iron'] * $config['worker'];
    $db->no_query("UPDATE `village` SET `arbwood` = '$arbwood', `arbclay` = '$arbclay', `arbiron` = '$arbiron', `arbmiliz` = '0', `arbnone` = '0' WHERE id = '$hid'");
    return true;
  }
